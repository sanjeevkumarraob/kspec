...
| `stage` | Derived lifecycle phase: `uninitialized`, `requirements`, `design`, `tasks`, `build`, `verify`, or `complete`. `complete` is terminal only until a later workflow-start event is recorded. It is derived from the `done` event in the spec's `metrics.json` and carries an empty `next.candidates`, so a scheduled consumer has an explicit stopping condition. A later start of specification, design, task generation, build, verification, fix, refactor, spike, or revision work reopens the derived lifecycle. A `tasks.md` that exists but contains no checkboxes reports `tasks`, not `verify`, because no work was ever scheduled. |
