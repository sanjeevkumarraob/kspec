...
function buildChatArgs(message, agent, passthroughArgs = [], options = {}) {
  ...
  // Both verified against kiro-cli 2.11: `--mode default|spec` selects the V3
  // agent mode (`spec` engages Kiro's native Spec agent); `--effort` is one of
  // low|medium|high|xhigh|max. Both are `chat` subcommand flags.
  if (options.mode) base.push('--mode', options.mode);
  if (options.effort) base.push('--effort', options.effort);
  // `--v3` is a GLOBAL flag and precedes the subcommand: `kiro-cli --v3 chat …`.
...
Update steering docs as needed.`;
  ...
  async spec(args) {
    ...
    // In V3, spec creation runs through Kiro's native Spec agent (`--mode spec`)
    // rather than a custom agent. See buildChatArgs / kiro-cli 2.11 `--mode`.
